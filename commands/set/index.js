...
const { isValid, coerceInput, revertCamelcase, closestSetting } = require(resolveLocal('./dep.js'))
...
const set = function (messageArgs, message, opts, cb) {
    ...
    let temp = info[closest]  // Temporary variable
    output = writeGuildInfo(id, closest, coercedVal)
    info[closest] = temp

    if (output instanceof Error) {
        if (!(output instanceof Error))
            output = null   // Mute client-side input errors
        cb(output, {
