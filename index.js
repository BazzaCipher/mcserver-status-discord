...
client.on('message', message => {
    ...
    if (!guild) {
        ...
        if (args[2] === 'help')
            return currentCommands.get('help')(args.slice(0, 3), message, returnHandler)
        if (!args[3])
            return currentCommands.get('_')(args, message, returnHandler)
    ...
    if (args[1] !== getGuildInfo(guild.id).prefix)
        return
